...
def fibonacci_memoized(n) -> int:
    ...
    if memo_fibonacci[n] != -1: 
        ...
        return memo_fibonacci[n]
# The Time Complexity : O(n) because each term is only calculated once and memoized for future use
# The Space Complexity : O(n) due to the recursion depth 
...
def main():
    ...
    # Input the Validation condition
    while (n < 0):
        n = int(input("Error!, Write a positive number: "))
